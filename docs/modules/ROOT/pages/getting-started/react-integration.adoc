= React Integration
:description: Integrating Hylograph Simulation with React components

Hylograph provides the `useSimulation` hook for React integration, matching the subscription pattern used in Halogen.

== Installation

React integration requires the separate `hylograph-react` package:
...
mkForceGraph = do
  ...
  component "ForceGraph" \props -> React.do
    ...
    { simulation, currentNodes, isRunning, tickCount } <-
      useSimulation mySetup props.nodes props.links

    -- Render when tick count changes (not currentNodes - see note below)
...
| `tickCount` | `Int` - counter incremented each tick (use as `useEffect` dependency)
|===

IMPORTANT: Use `tickCount` as your `useEffect` dependency, not `currentNodes`. PureScript's `Eq` for arrays compares *contents*, and D3 mutates nodes in place (same object references). This means `eq oldNodes newNodes` returns `true` even when positions change, so `useEffect currentNodes` won't re-run on ticks.
...
import Hylograph.React.Simulation (useSimulation)
import Hylograph.ForceEngine.Setup as Setup
...
mkForceGraph = do
  ...
  component "ForceGraph" \props -> React.do
    ...
    { currentNodes, isRunning, tickCount } <-
      useSimulation forceSetup props.nodes props.links

    -- Render SVG when tick count changes
    -- (tickCount is the correct dependency, not currentNodes)
    useEffect tickCount do
...
-- Rendering function (uses hylograph-selection or raw D3)
...
[source,purescript]
----
import Hylograph.React.Simulation (useSimulationWithCallbacks, defaultCallbackHandlers)
...
mkDraggableGraph = do
  ...
  component "DraggableGraph" \props -> React.do
    { simulation, currentNodes, tickCount } <- useSimulation setup props.nodes props.links
    ...
    useEffect tickCount do
      case simulation of
        Nothing -> pure mempty
        Just sim -> do
          renderNodes containerId currentNodes  -- First render
