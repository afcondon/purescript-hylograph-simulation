= Halogen Integration
:description: Integrating PSD3 Simulation with Halogen components

PSD3 Simulation provides first-class Halogen integration through the subscription pattern. This enables the "framework-first" architecture where all state lives in your Halogen component.

== Installation

Halogen integration requires the separate `psd3-simulation-halogen` package:
...
[source,purescript]
----
import PSD3.ForceEngine.Events (defaultCallbacks, SimulationCallbacks)
